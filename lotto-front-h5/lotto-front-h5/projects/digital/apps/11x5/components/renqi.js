...
export default class RENQI extends Component {
  ...
  ballRandom(highlight = false) {
    let balls = [];
    do {
      balls = this.panel.random(7, highlight);
    } while (!Common.checkLimit(balls, false, this.props.limitInfoList, 7));
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    ...
    return {
      ...
      label: `任七${fixedBalls.length ? '胆拖' : '普通'}投注 ${betNum}注`,
      ...
      if (!betNum) {
        if (!ballNumber && !this.state.fixedBalls.length) {
          ...
          confirm
            .confirm(
              <div>
                <p>还差{7 - ballNumber - this.state.fixedBalls.length}个号码</p>
                <p>才能组成一注彩票</p>
              </div>,
              '帮我补足',
              '自己选'
              ...
              do {
                newBalls = this.state.balls.concat(
                  this.panel.completion(
                    7 - ballNumber - this.state.fixedBalls.length
                  )
                );
              } while (
                !Common.checkLimit(
        ...
        if (!Common.checkLimit(balls, false, this.props.limitInfoList, 7)) {
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>
                从11个号码中任选5个号码。1/22的中奖机会,奖金<em>26</em>元！
              </span>
              <span>点两次可设胆 [胆码至少选1个，最多6个]</span>
            </p>
          }
        >
