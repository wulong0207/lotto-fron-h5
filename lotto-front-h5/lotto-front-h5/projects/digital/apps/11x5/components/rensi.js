...
export default class RENSI extends Component {
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    ...
    return {
      ...
      label: `任四${fixedBalls.length ? '胆拖' : '普通'}投注 ${betNum}注`,
      ...
      if (!betNum) {
        if (!ballNumber && !this.state.fixedBalls.length) {
          ...
          confirm
            .confirm(
              <div>
                <p>还差{4 - ballNumber - this.state.fixedBalls.length}个号码</p>
                <p>才能组成一注彩票</p>
              </div>,
              '帮我补足',
              '自己选'
              ...
              do {
                newBalls = this.state.balls.concat(
                  this.panel.completion(
                    4 - ballNumber - this.state.fixedBalls.length
                  )
                );
              } while (
                !Common.checkLimit(
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>
                从11个号码中任选4个号码。1/66的中奖机会,奖金<em>78</em>元！
              </span>
              <span>点两次可设胆 [胆码至少选1个，最多3个]</span>
            </p>
          }
        >
