...
export default class BH6 extends Component {
  ...
  ballChangeHandle(balls, _, index) {
    ...
    if (index === 0) {
      ...
      let bet = this.CALC({ betArr: [balls, newBalls], ind: 7, money: 160 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    let betNum = this.CALC({ betArr: [fixedBalls, balls], ind: 7, money: 160 })
      .num;
    if (!fixedBalls.length) {
      betNum = this.CALC({ betArr: [balls], ind: 6, money: 160 }).num;
    }
    return {
      betNum,
      manual,
