...
export default class EXGROUPSUM extends Component {
  constructor(props) {
    super(props);
    this.state = {
      balls: [[]],
    ...
    queue.subscribe(data => {
      ...
      let bet = this.CALC({ betArr: balls, ind: 12, money: 50 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true) {
    let betNum = this.CALC({ betArr: balls, ind: 12, money: 50 }).num;
    return {
      betNum,
      manual,
      balls: formatBall(balls, ' '),
      content: formatBall(balls, ','),
      label: `二星组选和值 ${betNum}注`,
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>所选和值号码与开奖号码后两位之和相同，即中奖50元</span>
            </p>
          }
        >
