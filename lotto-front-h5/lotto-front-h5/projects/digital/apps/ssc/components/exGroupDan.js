...
export default class BH6 extends Component {
  ...
  ballChangeHandle(balls, _, index) {
    if (index === 0) {
      ...
      let bet = this.CALC({ betArr: [balls, newBalls], ind: 11, money: 50 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    let betNum = this.CALC({ betArr: [fixedBalls, balls], ind: 11, money: 50 })
      .num;
    if (!fixedBalls.length) {
      betNum = this.CALC({ betArr: [balls], ind: 10, money: 50 }).num;
    }
    return {
      betNum,
      manual,
      ...
      label: `二星组选${fixedBalls.length ? '胆拖' : ''} ${betNum}注`,
