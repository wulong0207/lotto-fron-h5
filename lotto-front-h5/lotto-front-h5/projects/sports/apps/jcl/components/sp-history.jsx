...
class SPHistory extends Component{
    ...
    render(){
        let {data, showMode, selectData, betKind} = this.props;
        ...
        let titleArea, valueArea = [];
        let fieldItem = this.fieldSet[betKind];
        if(fieldItem){
            titleArea = fieldItem.titles.map((val, index)=>{
                if(val != "更新时间"){
                    return <li key={index} className="flex">{val}</li>
                }else{
                    return <li key={index} className="udpate-time">更新时间</li>;
                }
            });
            ...
            if(data){
                valueArea = data.map((dataItem, dataIndex)=>{
                    ...
                    return <ul key={dataIndex} className={"data-list data "+className}>
                    {
                        dataItem.map((val, index)=>{
                            if(index != dataItem.length - 1){
                                ...
                                return <li key={index} className="udpate-time">{this.getFullTime(val)}</li>;
                            }
                        })
                    }
                    </ul>
        ...
        return <div className="sp-history">
                <div className="take-position" onClick={this.show.bind(this)}></div>
                ...
                <div className="cart">
                    <p className="message"> {selectData.week} {selectData.num} {selectData.g_s_name} vs {selectData.h_s_name}</p>
                    <div className="slide">
...
const mapDispatchToProps = dispatch => {
  return {
      fetchSP(matchId, betKind) {return dispatch(fetchSP(matchId, betKind));},
      toggle(selectData, betKind){return dispatch(toggle(selectData, betKind));}
  }
}
