  ...
  return (
    <div className="status-bar">
      <p>
        ...
        <span>
          {
            ggType.map(i => i.split('*')[0]).sort((a, b) => a - b).map(g => {
              return (
                <span key={ g }>{ `${g}` }</span>
              )
            })
      ...
      <div className="total-summary">
        <div className="total-money-summary">
          ...
          <div className="total-bet-num">
            <h5 className="total">共<i>{ betNum * 2 }</i>元</h5>
          </div>
        </div>
        <button
          className="order-btn"
        ...
        >{ orderStatus === 'pending' ? '处理中' : '立即投注' }</button>
      </div>
    </div>
  )
};
...
export default class OptimizationComponent extends PureComponent {
  ...
  onSumChange(sum) {
    if (sum % 2) {
      const fn = () => {
        ...
        const max = betting.maxInfo.betNum * 2;
        if (sum + 1 < min) {
          num = min;
        } else if (sum + 1 > max) {
          num = max;
        ...
        this.numberInput.setState({number: num});
      };
  ...
  render () {
    ...
    const optimizeAvailable = betting.ggType.length === 1 && betting.ggType[0].split("串")[0] / 1 === groupArrayByKey(betting.selected, 'id').length;
      ...
      <div className="football-optimization">
        ...
        <div className="amount-money">
          ...
          <NumberInput
            ...
            max={ betting.maxInfo.betNum * 2 }
            messages={{
              max: '本次计划投注金额最大为'+betting.maxInfo.betNum * 2+'元',
              min: `本次计划投注金额至少为${this.props.betting.betNum * 2 * this.props.betting.times}元`
            }}
            step={ 2 }
        ...
        <div className="average optimization-item">
          <div
            className={ cx('optimization-title', {'selected': type === 'average'}) }
        ...
        <div className={cx('heat', 'optimization-item', {'disabled': !optimizeAvailable})}>
          <div
            className={ cx('optimization-title', {'selected': type === 'heat'}) }
            onClick={ this.setOptimizeType.bind(this, 'heat', !optimizeAvailable) }>
            <h5>博热优化</h5>
        ...
        <div className={cx('cold', 'optimization-item', {'disabled': !optimizeAvailable})}>
          <div
            className={ cx('optimization-title', {'selected': type === 'cold'}) }
            onClick={ this.setOptimizeType.bind(this, 'cold', !optimizeAvailable) }>
            <h5>博冷优化</h5>
