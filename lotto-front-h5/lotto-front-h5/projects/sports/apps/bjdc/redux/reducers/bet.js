...
const defaultState = {
  ...
  betCalc: savedData.betCalc || {
    // 胜平负
    spf: getDefaultBetCalc(),
    // 胜负过关
    sfgg: getDefaultBetCalc(),
    // 上下单双
    sxds: getDefaultBetCalc(),
    // 总进球数
    zjqs: getDefaultBetCalc(),
    // 全场比分
    qcbf: getDefaultBetCalc(),
    // 半全场
    bqc: getDefaultBetCalc()
  },
...
function isMatchLimitReached(state, max = 15) {
  ...
  if (gameCount.count > max) {
    Alert.alert({ msg: '单个方案订单最多选' + max + '场比赛' });
    return true;
  }
...
function closeLoadding() {
  ...
  if (calcingDOM) {
    calcingDOM.style.display = 'none';
  }
}

// 奖金计算
function calc(state, calDetail) {
  ...
  loopBets(state.bets, (item, field) => {
    ...
    // 获取SP值
    let betObj = getSP(gameDatas, field, selectList);
    // 获取单关的投注内容
  ...
  if (
    ...
    (mode === 'sfgg' && params.orderObj.list.length < 3)
  ) {
    result = { betNum: 0, maxBonus: 0 };
  } else {
    result = calDetail ? calcProfitDetail(params) : calcBet(params);
  }
