...
class Dan extends Component {
  ...
  getSubBetDetail(betArr, game, mode) {
    ...
    for (let i = 0; i < betArr.length; i++) {
      let item = betArr[i];
      let spSet = config[mode].sp;
      let sp = game[spSet.getVal ? spSet.getVal(item) : spSet.val];
      subDetails.push(
        <div key={ i } className="item">
          {config[mode][item]}@{sp[spSet[item]]}
        </div>
      );
    }
    ...
    return (
      <li key={ mode } className="sub-bet display-flex">
        <div className="title">
          {config[mode].getTitle
            ? config[mode].getTitle(game)
            : config[mode].title}
        </div>
  ...
  renderOptions() {
    ...
    loopBets(bets, (item, field) => {
      if (checkHasSelect(item, mode).hasSelect) {
        let game = getGame(data, field).game;
        let cln =
        ...
        let addDanArea = (
          <span className={ cln }>
            <img
              ...
              src={
                item.bravery[mode]
                  ? require('../img/icon_radio_sel.png')
                  : require('../img/icon_radio_unsel.png')
              }
            />
          </span>
        );
