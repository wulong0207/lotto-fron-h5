...
export class Jx11x5 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  getLotteryInfo() {
    // 彩种信息
    http
      .get(`/jx11x5/activity/yfgc/lottoinfo?t=${+new Date()}`)
      .then(res => {
  ...
  handleValidData(type) {
    ...
    if (type == 'active') {
      url = '/jx11x5/activity/yfgc/order';
    }
        ...
        orderAddContentList: [
          {
            ...
            lotteryChildCode: 21308, //    子玩法   ID   number  必选。
  ...
  handleErrorCode(err) {
    switch (err.code) {
      ...
      case '40312':
        ...
        // this.state.orderCode = err.data.orderAddCode || '';
        Message.alert({
  ...
  goToJjcLottery(code) {
    let token = session.get('token');
    // chase-lottery?orderCode=D17101515442516600080&lotteryCode=210&buyType=1&lotteryChildCode=210
    // location.href='sc.html#/chase-lottery?orderCode=' + code  + "&lotteryCode=210&buyType=1&lotteryChildCode=210"
    // sc.html#/orders/D17110818172016600007
    // location.href = 'sc.html#/orders/' + code;
  }
