...
export function calculateBetNumAndMaxProfit(bettings, times) {
  ...
  if (betNum !== 0) {
    ...
    const minValues = values.map(v => {
      return Math.min(...v.map(i => parseFloat(i.split('@')[1])));
    });
    return {
      betNum,
      max: maxValues
