...
export function getSingleMatchs(data) {
  return data.reduce((acc, d) => {
    let arr = acc.concat();
    const singleMatchs = d.matchs.filter(m =>
...
export function generateAllScoreData(match) {
  ...
  const wScoreArr = generateScoreSize()
    .concat()
    .map((score, i) => {
      let data = {
        score: score
...
// 检查数组是否相等
export function equalArr(arrA, arrB) {
  let result = false;
    ...
    if (arrA.length == arrB.length) {
      if (arrA.length == 0) {
        ...
        for (let i = 0; i < arrA.length; i++) {
          if (arrA[i] != arrB[i]) {
            compareResult[arrA[i]] = !compareResult[arrA[i]];
            compareResult[arrB[i]] = !compareResult[arrB[i]];
          }
        }

        for (let item in compareResult) {
