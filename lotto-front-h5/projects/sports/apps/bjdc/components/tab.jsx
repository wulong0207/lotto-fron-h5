...
class MenuItem extends Component {
  ...
  renderMenu(subMenu) {
    ...
    for (let i = 0; i < subMenu.length; i++) {
      let item = subMenu[i];
      let td = (
        <td onClick={ this.onMenuItemClick.bind(this, item) } key={ i }>
          {item.des}
        </td>
      );
      tds.push(td);
    ...
    if (tds.length > 0) {
      tds.push(<td key={ subMenu.length } />);
      subAreas.push(<tr key={ subMenu.length }>{tds}</tr>);
    }

    return (
      <table>
        <tbody>{subAreas}</tbody>
      </table>
    );
  }
  ...
  render() {
    ...
    return (
      <div className="sub-menu" onClick={ this.showArea.bind(this) }>
        <div className="des">
          {des}
          {menus && menus.length > 0 ? <span className="arrows" /> : ''}
        </div>
    ...
    return (
      <div className="menu-tab">
        <table cellPadding="0" cellSpacing="0">
          <tbody>
            <tr className="display-flex">{this.renderMenu(tabs)}</tr>
          </tbody>
        </table>
      </div>
    );
