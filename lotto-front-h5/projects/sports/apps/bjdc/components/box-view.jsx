...
export default class BoxView extends Component {
  ...
  onScrollHandler() {
    ...
    const BVHeadHeight = this.header.clientHeight;
  ...
  show(shouldShow) {
    this.childArea.style.transition = 'all 0.5s';
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h3 className="title">
            {title}
            <span className={ 'toggle flex ' + (open ? '' : 'noshow') }>
              <span className="arrows" />
            </span>
          </h3>
          <span className="flex" />
        </div>
        <div ref={ childArea => this.childArea = childArea } className={ 'child-area ' + ca }>
          {this.props.children}
        </div>
      </div>
    );
