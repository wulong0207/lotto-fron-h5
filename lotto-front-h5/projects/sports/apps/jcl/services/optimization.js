...
export function bettingCompose({ selected, ggType, courage }) {
  ...
  if (courage && courage.length) {
    ...
    // 设胆的排列组合
    const courageCombination = compose_iter(groupArrayByKey(courageBetting, 'id').map(b => b.data));
    // console.log('courageCombination', courageCombination.toArray());
    // // 胆拖的排列组合
    const combinations = compose({ selected: bettings.filter(s => courage.indexOf(s.id) < 0), ggType: ggType.map(i => i - courage.length) });
    // console.log('combination', combination);
    return compose_iter([courageCombination, combinations]).map(i => flatten(i));
  } else {
...
export function generateCombinations(betting) {
  return bettingCompose({
    ...
    const sp = c.reduce((acc, i) => {
      return acc.times(i.sp);
    }, new Decimal(1)).times(2).toFixed(2);
    return {
      times: 1,
      combinations: c,
...
export function average(combs, sum) {
  ...
  // 先把每一注单注奖金的  基数相加
  const totalSp = combs.reduce((acc, c) => {
...
function changeCombinationTime(combinations, positive, length) {
  let combs = [];
  const combLength = positive ? combinations.length : combinations.filter(c => c.times > 1).length;
