...
export default class Match extends Component{
    ...
    componentWillReceiveProps(nextProps) {
        if(nextProps.select && (nextProps.select.left !== this.state.select.left
    ...
    getCenter(){
        let {data, field} = this.props;
        let centerArea = <li className="display-flex des-center"
        ...
        if(field){
            ...
            if(this.fieldsSet[field] && centerConfig){
                ...
                centerArea = <li className={"display-flex des-center "+cn}
                                 onClick={this.onCenterClickHandler.bind(this)}>
                    {result}
                </li>
            }
    ...
    render(){
        ...
        if(sellMark == 1){
            if(field == "sf" && ( data.statusWf == 2 || data.statusWf == 4)){
                isSell = false;
            }else if(field == "rfsf" && ( data.statusLetWf == 2 || data.statusLetWf == 4)){
                isSell = false;
            }else if(field == "dxf" && ( data.statusBigSmall == 2 || data.statusBigSmall == 4)){
                isSell = false;
            }
        }else if(sellMark == 2){
        ...
        return <div className={"match-bar "+linebox}>
            {isSell?<ul className="display-flex">
                <li className={"flex display-flex "+lc}
                ...
                {this.getCenter()}
                <li className={"flex display-flex "+rc}
