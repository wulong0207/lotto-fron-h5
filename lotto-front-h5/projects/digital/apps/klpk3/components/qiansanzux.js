...
export default class QIANSANGROUP extends Component {
  constructor(props) {
    super(props);
    this.state = {
      balls: [],
  ...
  ballRandom(highlight = false) {
    let balls = [];
    do {
      balls = this.panel.random(3, highlight);
      console.log(Common.checkLimit(balls, true, this.props.limitInfoList, 3));
    } while (!Common.checkLimit(balls, true, this.props.limitInfoList, 3));
    return balls;
  }
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    ...
    return {
      ...
      label: `前三组选${fixedBalls.length ? '胆拖' : '普通'} ${betNum}注`,
      ...
      if (!betNum) {
        if (!ballNumber && !this.state.fixedBalls.length) {
          ...
          confirm
            .confirm(
              <div>
                <p>还差{3 - ballNumber - this.state.fixedBalls.length}个号码</p>
                <p>才能组成一注彩票</p>
              </div>,
              '帮我补足',
              '自己选'
              ...
              do {
                newBalls = this.state.balls.concat(
                  this.panel.completion(
                    3 - ballNumber - this.state.fixedBalls.length
                  )
                );
              } while (
                !Common.checkLimit(
        ...
        if (!Common.checkLimit(balls, false, this.props.limitInfoList, 3)) {
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>
                任意3个号码组成1注有效注。1/165的中奖机会,奖金<em>195</em>元！
              </span>
              <br />
              <span>点两次可设胆 [胆码至少选1个，最多2个]</span>
            </p>
          }
        >
