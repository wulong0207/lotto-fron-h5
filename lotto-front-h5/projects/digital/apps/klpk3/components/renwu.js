...
export default class RENWU extends Component {
  ...
  editHandle(content) {
    ...
    let str = [];
    let c;
    if (content.indexOf('YYY') > 0 || content === 'YYY') {
      ball = content.split(',');
      ball.pop();
      balls[1].push('豹子全包');
  ...
  render() {
    ...
    if (ballNumber === 13) {
      ...
      amount = 500 + '~' + 6900;
      profit = 500 - betNum * 2 + '~' + (6900 - betNum * 2);
