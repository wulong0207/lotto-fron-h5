...
export default class RENQI extends Component {
  constructor(props) {
    ...
    queue.subscribe(data => {
      ...
      let betNum = getCombinationBetNum(balls[0], 2);
  ...
  ballRandom(highlight = false) {
    let balls = [[]];
    do {
      let ball = this.panel0.random(2, highlight);
      balls[0] = ball;
  ...
  render() {
    ...
    if (betNum === 0) {
      ...
      amount = 33 + '~' + 99;
      profit = 33 - betNum * 2 + '~' + (99 - betNum * 2);
    }
    return (
