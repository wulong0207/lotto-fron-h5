...
export default class QIANSANSELECT extends Component {
  constructor(props) {
    ...
    queue.subscribe(data => {
      ...
      let bet = this.CALC({ betArr: balls, ind: 10, money: 50 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true) {
    let betNum = this.CALC({ betArr: balls, ind: 10, money: 50 }).num;
    return {
      betNum,
      manual,
            ...
            .then(() => {
              ...
              do {
                ...
                newBalls.map((item, index) => {
                  item.push(this.panel.random(1, false, newBalls).toString());
                });
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>选2个号码，猜对开奖后2位（顺序不限）即中50元</span>
            </p>
          }
        >
