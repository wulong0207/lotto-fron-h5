...
export default class QIANSANSELECT extends Component {
  constructor(props) {
    ...
    queue.subscribe(data => {
      ...
      let bet = this.CALC({ betArr: balls, ind: 3, money: 1000 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true) {
    let betNum = this.CALC({ betArr: balls, ind: 3, money: 1000 }).num;
    return {
      betNum,
      manual,
      balls: formatBall(balls, ' '),
      content: formatBall(balls, ','),
      label: `三星直选和值 ${betNum}注`,
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>所选和值与开奖号码后三位之和相同，即中奖一千元</span>
            </p>
          }
        >
