...
export default class QIANSANSELECT extends Component {
  constructor(props) {
    ...
    queue.subscribe(data => {
      ...
      let bet = this.CALC({ betArr: balls, ind: 6, money: 160 });
      let betNum = bet.num;
  ...
  generateBet(balls, manual = true) {
    let betNum = this.CALC({ betArr: balls, ind: 6, money: 160 }).num;
    return {
      betNum,
      manual,
        ...
        if (balls[0].length === 0) {
          ...
          confirm
            ...
            .then(() => {
              let newBalls = [];
              do {
                let b = this.panel.completion(3 - balls[0].length);
                newBalls = this.state.balls[0].concat(b);
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>选3个号码，开奖号码后三位为组六号且包含选中即中160元</span>
            </p>
          }
        >
