...
export default class RENWU extends Component {
  constructor(props) {
    super(props);
    this.state = {
      balls: [],
  ...
  ballRandom(highlight = false) {
    let balls = [];
    do {
      balls = this.panel.random(5, highlight);
    } while (!Common.checkLimit(balls, false, this.props.limitInfoList, 5));
    return balls;
  }
  ...
  generateBet(balls, manual = true, fixedBalls = []) {
    ...
    return {
      ...
      label: `任五${fixedBalls.length ? '胆拖' : '普通'}投注 ${betNum}注`,
      ...
      if (!betNum) {
        if (!ballNumber && !this.state.fixedBalls.length) {
          ...
          confirm
            .confirm(
              <div>
                <p>还差{5 - ballNumber - this.state.fixedBalls.length}个号码</p>
                <p>才能组成一注彩票</p>
              </div>,
              '帮我补足',
              '自己选'
              ...
              do {
                newBalls = this.state.balls.concat(
                  this.panel.completion(
                    5 - ballNumber - this.state.fixedBalls.length
                  )
                );
              } while (
                !Common.checkLimit(
        ...
        if (!Common.checkLimit(balls, false, this.props.limitInfoList, 5)) {
        ...
        <BetInfo
          ...
          emptyNode={
            <p className="empty-tip">
              <span>
                从11个号码中任选5个号码。1/462的中奖机会,奖金<em>540</em>元！
              </span>
              <span>点两次可设胆 [胆码至少选1个，最多4个]</span>
            </p>
          }
        >
