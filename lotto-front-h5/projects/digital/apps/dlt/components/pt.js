...
export default class PT extends Component {
  ...
  ballRandom(highlight = false) {
    let redBall = this.panel0.random(5, highlight);
    let blueBall = this.panel1.random(2, highlight);
    ...
    return new Promise((resolve, reject) => {
      if (!betNum) {
        if (!redNum && !blueNum) {
          ...
          let content = '';
          if (5 - redNum > 0 && 2 - blueNum > 0) {
            content = `还差${5 - redNum}个红球和${2 - blueNum}个篮球`;
          } else if (5 - redNum > 0) {
            content = `还差${5 - redNum}个红球`;
          } else {
            content = `还差${2 - blueNum}个篮球`;
          }
          confirm
            .confirm(
              <div>
            ...
            .then(() => {
              ...
              for (let i = 0; i < balls.length; i++) {
                if (i === 0) {
                  if (5 - balls[i].length > 0) {
                    balls[i] = balls[i]
                      .concat(this['panel' + i].completion(5 - balls[i].length))
                      .sort();
                  }
                } else {
                  if (balls[i].length < 2) {
                    balls[i] = balls[i]
                      .concat(this['panel' + i].completion(2 - balls[i].length))
                      .sort();
                  }
                }
              }
