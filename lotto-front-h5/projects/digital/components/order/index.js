...
export default class OrderComponent extends PureComponent {
  ...
  newOrder(data) {
    ...
    const url = chase ? '/chase/addChase' : '/order/addOrder';
    if (!data.token) {
      return goLogin();
    }
  ...
  multiplePay() {
    ...
    if (browser.yicaiApp) {
      ...
      window.location.href = `/pay.html?orderCode=${orderCode}&buyType=${
        buyType
      }&token=${window.sessionStorage.getItem('token')}`;
    }
  }
      ...
      >
        <div>
          ...
          <footer className="footer">
            ...
            >
              立即支付 {getAmount(this.state.selectedOrders, this.state.orders)}{' '}
              元
            </div>
          </footer>
        </div>
...
OrderComponent.propTypes = {
  ...
  detail: ReactComponent // 订单详情的模板，请传 React 组件，会回传 order 属性
};
